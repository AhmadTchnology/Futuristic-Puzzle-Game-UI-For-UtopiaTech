...
export const PersonalityAnalysis: React.FC<PersonalityAnalysisProps> = ({ startTime, endTime, onContinue, onClose, isReplay }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">
      ...
      >
        {/* Header / ID Card Top */}
        <div className="relative h-24 sm:h-32 overflow-hidden bg-gradient-to-b from-white/5 to-transparent p-4 sm:p-6">
          <div className="absolute top-0 right-0 p-4 opacity-20">
            <User size={80} className="sm:w-[100px] sm:h-[100px]" color={archetype.color} />
          </div>
          <div className="relative z-10">
            <div className="font-mono text-[8px] sm:text-[10px] tracking-widest text-white/50">IDENTITY_ANALYSIS_COMPLETE</div>
            <h2 className="mt-1 sm:mt-2 text-2xl sm:text-3xl font-black italic tracking-tighter text-white" style={{ textShadow: `0 0 20px ${archetype.color}` }}>
              {archetype.title}
            </h2>
            <div className="mt-1 font-mono text-lg sm:text-xl text-white/80">
        ...
        <div className="grid gap-4 sm:gap-6 p-4 sm:p-6">
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center rounded-lg bg-white/5 shrink-0" style={{ color: archetype.color }}>
              {archetype.icon}
            </div>
            <div>
